...
func (s storage) CreateMessageGroup(msgGroup *repository.MessageGroup) error {
	...
	exec, err := s.db.Exec(query, msgGroup.KzMsg, msgGroup.RuMsg, msgGroup.EnMsg)
	if err != nil {
		return fmt.Errorf("create message group was failed: %w", err)
	}
...
func (s storage) GetMessageGroup(msgID int, lang string) (repository.MessageGroup, error) {
	...
	err := row.Scan(&msgGroup.ID, &msgGroup.KzMsg, &msgGroup.RuMsg, &msgGroup.EnMsg)
	if err != nil {
		return repository.MessageGroup{}, fmt.Errorf("get message group was failed: %w", err)
	}
...
func (s storage) GetAllMessageGroups() ([]repository.MessageGroup, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get all messageGroup was failed:%w", err)
	}
	...
	for rows.Next() {
		msgGroup := repository.MessageGroup{}
		err = rows.Scan(&msgGroup.ID, &msgGroup.KzMsg, &msgGroup.RuMsg, &msgGroup.EnMsg)
		if err != nil {
			return nil, fmt.Errorf("get all messageGroup was failed:%w", err)
		}
...
func (s storage) DeleteMessageGroup(msgGroupID int) error {
	...
	if err != nil {
		return fmt.Errorf("delete messageGroup was failed:%w", err)
	}
